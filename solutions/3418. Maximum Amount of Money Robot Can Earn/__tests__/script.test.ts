...
describe('3418. Maximum Amount of Money Robot Can Earn', (): void => {
    it('maximum coins when neutralizing one robber on optimal path', (): void => {
        expect(maximumAmount([[0, 1, -1], [1, -2, 3], [2, -3, 4]]))
            .toBe(8)
    })

    it('sum of all positive cells when no robbers present', (): void => {
        expect(maximumAmount([[10, 10, 10], [10, 10, 10]]))
            .toBe(40)
    })

    it('single cell value for 1x1 grid with positive coin', (): void => {
