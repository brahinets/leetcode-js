...
describe('174. Dungeon Game', (): void => {
    it('returns 7 for the example dungeon', (): void => {
        expect(calculateMinimumHP([[-2, -3, 3], [-5, -10, 1], [10, 30, -5]])).toBe(7)
    })

    it('returns 1 when single cell has positive value', (): void => {
        expect(calculateMinimumHP([[5]])).toBe(1)
    })

    it('returns 2 when single cell has value -1', (): void => {
        expect(calculateMinimumHP([[-1]])).toBe(2)
    })

    it('returns 1 when all cells are positive', (): void => {
    ...
    it('returns correct answer when path forces taking damage', (): void => {
